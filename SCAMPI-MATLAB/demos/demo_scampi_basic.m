...
for i_snr=1:length(isnr)
   ...
   for iter = 1:N_iter;  
      ...
      %H_beam = beamspace_channel_lens_test(N1,N2,K,L,[],[],D_y,D_z,A);    % beamspace channel
      H_beam =test(N1,N2,K,L,[],[],D_y,D_z,A);
     for j = 1 : K
...
% noise
nvar = mean(y(y < max(abs(y) ) ).^2) * exp(-log(10) * isnr(i_snr) / 10); % noise variance (avoiding to take into account the measurement associated to the first Hadamard mode)
nvar
noise = sqrt(nvar) * randn(M,1); % zero mean additive white Gaussian noise    
y = y + noise; 
yz = [y; sparse(l, 1) ]; % augmented measurement
noise2 = sqrt(nvar) * randn(Q,1); 
% ssd
...
xx = Phi*h + 400*noise2;
[h_hat2 n1 n2 support r] = SSD_simplify(xx,Phi,L,N1,N2,real(h),V1,V2);
...
figure(1)
semilogy(isnr,NMSE1,'b','Linewidth',1.5);%gtext('Scampi');
hold on 
semilogy(isnr,NMSE2,'r','Linewidth',1.5);%gtext('SSD');
grid on
xlabel('SNR (dB)');
ylabel('NMSE (dB)');

% figure(2)
% plot(isnr,nsnr1,'b','Linewidth',1.5);gtext('Scampi');
% hold on 
% plot(isnr,nsnr2,'r','Linewidth',1.5);gtext('SSD');
% grid on
% xlabel('ISNR (dB)');
% ylabel('NSNR (dB)');