...
[W_aug, D] = augment_matrix(N1,N2,N,M,l,W_r);
...
% scoampi 
sigPow(1:N) = mean(x(1:N))^2;
sigPow(N+1:N+l) = mean(x(N+1:N+l))^2;
if (isrow(sigPow) ); sigPow = sigPow'; end
[X_scampi, D_scampi,prior] = oamp_vec_solver(yz, op, opTr, opSq, opSqTr, opt,isnr(i_snr),W_aug,sigPow,N,l,nvar); % X_scampi is the reconstructed image, D_scampi, the estimation of the dual variables
